// from https://github.com/PrincetonUniversity/MosaicSim
...
public:
  ...
  /** \brief   */
  void insert(uint64_t address, uint64_t offset, uint64_t setid,
...
public:
  ...
  void insert(uint64_t address, int nodeId, bool isLoad, int *dirtyEvict,
              int64_t *evictedAddr, uint64_t *evictedOffset, int *evictedNodeId, uint64_t* evictedSetid);
  ...
  uint64_t getSetid(uint64_t address);
};
