...
public:
    ...
    virtual void setRegion(Region* region,  address_t input_address, address_t filter_address, address_t output_address, Dataflow dataflow) {assert(false);}
    ...
    virtual bool is_compute_finished() { assert(false); }
    ...
    virtual void preload(unsigned int T_N, unsigned int T_K, unsigned int offset, Command *cmd){assert(false);}
    virtual void MatrixMultiply(unsigned int T_M, unsigned int T_K, unsigned int T_N, unsigned int _offset_to_MK, unsigned int _offset_to_MN, unsigned int _offset_to_KN, Command *cmd){assert(false);}
    virtual bool is_idle() {assert(false);}

    virtual Command* get_command() { assert(false); return nullptr;}

    virtual void config(address_t MK_address, address_t KN_address,
