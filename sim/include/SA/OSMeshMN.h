...
public:
    ...
    void preload(){for(auto& ms_pair : mswitchtable) {ms_pair.second->preload();}}
    void MatrixMultiply(tile_t T_M, tile_t T_K, tile_t T_N){for(auto& ms_pair : mswitchtable) {ms_pair.second->MatrixMultiply(T_M, T_K, T_N);}}
    void MatrixMultiplyStay(tile_t T_M, tile_t T_K, tile_t T_N){for(auto& ms_pair : mswitchtable) {ms_pair.second->MatrixMultiplyStay(T_M, T_K, T_N);}}
    void MatrixMultiplyFlip(tile_t T_M, tile_t T_K, tile_t T_N){for(auto& ms_pair : mswitchtable) {ms_pair.second->MatrixMultiplyFlip(T_M, T_K, T_N);}}
    
    bool psums_received();
    void reset_psums_received();
    bool is_compute_finished();
    void reset_compute_finished() override;
    bool has_array_flipped();
};
#endif 
