#ifndef __WSMESHSDMEMORY__H__
#define __WSMESHSDMEMORY__H__
...
class WSMeshSDMemory : public MemoryController {
private:
    DNNLayer* dnn_layer; // Layer loaded in the accelerator
    Region* region; // Layer loaded in the accelerator
    ...
    unsigned int offset_to_KN;
    unsigned int offset_to_MK;
    ...
    WSMeshControllerState w_current_state; //Stage to control what to do according to the state
    WSMeshControllerState a_current_state; //Stage to control what to do according to the state
    ...
    bool region_loaded; //Flag that indicates whether the layer has been loaded.
...
public:
    WSMeshSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection);
    ~WSMeshSDMemory();
    ...
    Command* get_command() { return this->currentCmd; }
