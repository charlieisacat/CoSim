...
PREDICTOR::PREDICTOR()
{
    ...
    for (int j = 0; j < 2; j++)
    {
        for (int i = 0; i < NUMTAGTABLES; i++)
        {
            ...
            // tagComp[j][i].targetLength = tagTableTagWidths[i]; // default value
            if (j == 0)
            {
                if (i < 2)
...
bool PREDICTOR::GetPrediction(uint32_t PC)
{
    ...
    for (int i = 0; i < NUMTAGTABLES; i++)
    {
        ...
        tag[i] &= ((1 << tagComp[0][i].targetLength) - 1);
    }
    // tag[0] &= ((1 << 9) - 1);
    // tag[1] &= ((1 << 9) - 1);
    // tag[2] &= ((1 << 9) - 1);
    ...
    for (int i = 0; i < NUMTAGTABLES; i++)
    {
        // indexTagPred[i] = PC ^ (PC >> (TAGPREDLOG)) ^ indexComp[i].compHist ^ PHR ^ (PHR >> (TAGPREDLOG));
        indexTagPred[i] = indexTagPred[i] & index_mask;
        // indexTagPred[i] = PC ^ (PC >> TAGPREDLOG - i) ^ indexComp[i].compHist ^ F(PHR, 16, i);
    }
...
void PREDICTOR::UpdatePredictor(uint32_t PC, bool resolveDir, bool predDir, uint32_t branchTarget)
{
    ...
    if ((!new_entry) || (new_entry && (primePred != resolveDir)))
    {
        if (((predDir != resolveDir) & (primeBank > 0)))
        {
            ...
            else
            {
                ...
                int randNo = rand() % 100;
                int count = 0;
                int bank_store[NUMTAGTABLES];// = {-1, -1, -1};
...
#if 0
void PREDICTOR::TrackOtherInst(uint32_t PC, uint32_t opcode, uint32_t branchTarget){
