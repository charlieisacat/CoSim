...
void OSMeshMN::setActivationLimit(unsigned int limit) {
    for(std::map<std::pair<int,int>, MultiplierOS*>::iterator it=mswitchtable.begin(); it!=mswitchtable.end(); ++it) {
...
void OSMeshMN::configureSignals(Tile* current_tile, Region* region, unsigned int ms_rows, unsigned int ms_cols) {
...
void OSMeshMN::reset_psums_received() {
    for (int i = (this->ms_rows - 1); i >= 0; i--) {
        for (int j = (this->ms_cols - 1); j >= 0; j--) {
            std::pair<int, int> ms_index(i, j);
            MultiplierOS* ms = mswitchtable[ms_index];
            ms->reset_psums_received();
        }
    }
}

bool OSMeshMN::psums_received() {
    bool received = true;
    for (int i = (this->ms_rows - 1); i >= 0; i--) {
        for (int j = (this->ms_cols - 1); j >= 0; j--) {
            std::pair<int, int> ms_index(i, j);
            MultiplierOS* ms = mswitchtable[ms_index];
            received = received && ms->is_psums_received();
        }
    }
    return received;
}

bool OSMeshMN::is_compute_finished() {
    std::pair<int, int> ms_index(this->ms_rows - 1, this->ms_cols - 1);
    return mswitchtable[ms_index]->is_compute_finished();
}

void OSMeshMN::reset_compute_finished() {
    std::pair<int, int> ms_index(this->ms_rows - 1, this->ms_cols - 1);
    mswitchtable[ms_index]->reset_compute_finished();
}

void OSMeshMN::reset_fifos() {
    for (int i = (this->ms_rows - 1); i >= 0; i--) {
        for (int j = (this->ms_cols - 1); j >= 0; j--) {
            std::pair<int, int> ms_index(i, j);
            MultiplierOS* ms = mswitchtable[ms_index];
...
bool OSMeshMN::has_array_flipped() {
