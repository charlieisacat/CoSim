...
DSwitch::DSwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config stonne_cfg, unsigned int ms_size) : Unit(id, name)
{ // ms_size depends on the local ms_size in the subtree
    this->level = level;
...
*/
void DSwitch::route_packages()
{ // TODO It is supposing you have enouth bandwidth in the connection
    ...
    {                                     // For every package in the connection
        ...
        if (pck->isBroadcast())
        { // If the package is broacast, no routing is needed.
        ...
        else if (pck->isUnicast())
        {
            ...
            { //(pck->get_unicast_dest() > half_ms_to_route)
                DataPackage *pck_right = new DataPackage(pck);
                output_right.push_back(pck_right);
            }
        }
        ...
        else if (pck->isMulticast())
        {
            ...
            {
                if (dests[i])
                { // If ms i needs to receive the message
                    route_left = true;
            ...
            for (int i = half_lim + 1; i < last_ms_to_route; i++)
            { // The same for the right part. Notice the index are correct
                if (dests[i])
                {
            ...
            if (route_left)
            { // Pushing the package into the output left link
                DataPackage *pck_left = new DataPackage(pck);
                output_left.push_back(pck_left);
            }

            if (route_right)
            { // Pushing the package into the output right link
                // auto start = std::chrono::steady_clock::now();
                DataPackage *pck_right = new DataPackage(pck);
                output_right.push_back(pck_right);
                //  auto  end = std::chrono::steady_clock::now();
    ...
    if ((output_left.size() > 0) && (output_right.size() > 0))
    {                                      // Broadcast
    ...
    if (output_left.size() > 0)
    { // If in the left branch there is something to send
        this->send(output_left, this->leftConnection);
        this->dswitchStats.n_left_sends++;
    }
    if (output_right.size() > 0)
    { // If in the right branch there is something to send
