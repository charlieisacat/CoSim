...
#include "SA/WSMeshSDMemory.h"
#include <assert.h>
#include <iostream>
#include "SA/utility.h"

WSMeshSDMemory::WSMeshSDMemory(id_t id, std::string name, Config stonne_cfg, Connection *write_connection) : MemoryController(id, name)
{
    ...
    this->a_current_state = WS_IDLE;
    this->w_current_state = WS_IDLE;
...
void WSMeshSDMemory::setWriteConnections(std::vector<Connection *> write_port_connections)
{
...
void WSMeshSDMemory::setReadConnections(std::vector<Connection *> read_connections)
{
...
void WSMeshSDMemory::config(address_t MK_address, address_t KN_address,
...
void WSMeshSDMemory::preload(unsigned int T_N, unsigned int T_K, unsigned int offset, Command *cmd)
{
    // assert((offset % T_N == 0) && (offset % T_K == 0));
    w_current_state = WS_DIST_WEIGHT;
    offset_to_KN = offset;
    this->cols_used = T_N;
    this->k_used = T_K;
    // this->current_K = T_K - 1;
    this->current_K = ms_cols - 1;
    this->current_N = 0;
    this->currentCmd = cmd;
    this->T_N = T_N;
    this->T_K = T_K;
...
void WSMeshSDMemory::MatrixMultiply(unsigned int T_M, unsigned int T_K, unsigned int T_N, unsigned int _offset_to_MK, unsigned int _offset_to_MN, unsigned int _offset_to_KN, Command *cmd)
{
    // assert((_offset_to_MK % T_M == 0) && (_offset_to_MK % T_K == 0));
    // assert((_offset_to_MK % T_K == 0));
    a_current_state = WS_DIST_INPUTS;
...
bool WSMeshSDMemory::is_idle() {
    return this->a_current_state == WS_IDLE;
}

void WSMeshSDMemory::cycle()
{
    ...
    if (w_current_state == WS_DIST_WEIGHT)
    {
        // std::cout<<"Distributing weights, current_K: "<<this->current_K<<std::endl;
        if (this->current_K > this->T_K - 1) {
            for (int i = 0; i < this->ms_rows; i++) {
                data_t data = 0;
                DataPackage* pck_to_send = new DataPackage(sizeof(data_t), data, WEIGHT, 0, UNICAST, i);
                // std::cout << "index=" << -1 << " offset_to_KN: " << offset_to_KN << " current_K: " << this->current_K << " data:" << data << std::endl;
            ...
            for (int i = 0; i < this->ms_rows; i++) {
                ...
                // padding
                if (i < this->cols_used) {
                    // std::cout<<"offset_to_KN: "<<offset_to_KN<<" current_K: "<<this->current_K<<" i: "<<i<<" K: "<<K<<std::endl;
                    // assert(offset_to_KN + i * K + this->current_K < (N*K));
                    offset = offset_to_KN + i * K + this->current_K;
                    // data = this->KN_address[offset];  // Notice that in dense operation the KN matrix is actually NK
                }
                // std::cout << "index=" << offset << " offset_to_KN: " << offset_to_KN << " current_K: " << this->current_K << " data:" << data << std::endl;
        ...
        // this->current_K++;
        // std::cout << "current_K: " << this->current_K << " T_K: " << this->k_used << std::endl;
        // if (this->current_K == this->k_used)
        if (this->current_K == 0)
        {
            // execution_finished = true;
            w_current_state = WS_IDLE;
        } else {
            this->current_K--;
        }
        // std::cout << "current_K: " << this->current_K << " T_K: " << this->k_used << std::endl;
    ...
    if (a_current_state == WS_DIST_INPUTS)
    {
        // if(this->current_M < this->k_used) {
            for (int k = 0; k < this->ms_rows; k++)
            {
                data_t data = 0;
                // implicit padding
                if(k < this->k_used) {
                    int offset = offset_to_MK + this->current_M * K + k;
                    // assert(offset < (M*K));
                    // data = this->MK_address[offset]; // Notice that in dense operation the KN matrix is actually NK
                }
                // std::cout << "offset_to_MK: " << offset_to_MK << " current_M: " << this->current_M << " k: " << k + this->ms_cols << " data:" << data << std::endl;
                sdmemoryStats.n_SRAM_input_reads++;
                DataPackage *pck_to_send = new DataPackage(sizeof(data_t), data, IACTIVATION, 0, UNICAST, k + this->ms_cols);
                this->sendPackageToInputFifos(pck_to_send);
            }
        // }
        // else {
        ...
        this->current_M++;
        // std::cout << "current_M: " << this->current_M << " rows_used: " << this->rows_used << std::endl;
        // if (this->current_M == ms_cols)
        if (this->current_M == this->rows_used)
        {
            a_current_state = WS_IDLE;
        }
    }
...
bool WSMeshSDMemory::isExecutionFinished()
{
...
void WSMeshSDMemory::sendPackageToInputFifos(DataPackage *pck)
{
    // BROADCAST PACKAGE
    if (pck->isBroadcast())
    {
...
{ // TODO control if there is no space in queue
    if (this->write_connection->existPendingData())
    {
        ...
        for (int i = 0; i < data_received.size(); i++)
        {
            ...
            // std::cout<<"receieve cycle="<<local_cycle <<" num="<<data_received[i]->get_source()<<" "<<data_received[i]->get_data()<<std::endl;
        }
    }
...
void WSMeshSDMemory::printStats(std::ofstream &out, unsigned int indent)
{
...
void WSMeshSDMemory::printEnergy(std::ofstream &out, unsigned int indent)
{
    /*
