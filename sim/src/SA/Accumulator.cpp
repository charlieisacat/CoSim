...
#include "SA/Accumulator.h"
#include <iostream>
#include <assert.h>
#include <math.h>
#include "SA/DataPackage.h"
...
void Accumulator::send()
{
    if (!output_fifo->isEmpty())
    {
        ...
        while (!output_fifo->isEmpty())
        {
            ...
            std::cout << "[ACCUMULATOR_FUNC] Cycle " << local_cycle << ", Accumulator " << this->n_accumulator << " has sent a psum to memory (FORWARDING DATA)" << " num=" << pck->get_source() << std::endl;
        }
...
void Accumulator::receive()
{
    ...
    {                                                                                // If there is data to receive on the left
...
DataPackage *Accumulator::perform_operation_2_operands(DataPackage *pck_left, DataPackage *pck_right)
{
    ...
    assert(pck_left->get_source() == pck_right->get_source()); // vn must fit
    ...
    DataPackage *result_pck = new DataPackage(sizeof(data_t), result, PSUM, pck_left->get_source(), pck_left->get_vn(), this->operation_mode); // TODO the size of the package corresponds with the data size
...
void Accumulator::route()
{
    ...
    if (!input_fifo->isEmpty())
    {
        ...
        DataPackage *result;
        if (current_psum == 0)
        { // There is no package yet to sum in this iteration
            // Creating package 0
            // std::cout << "[ACCUMULATOR] Accumulator received PSUM: " << pck_received->get_data() << " vn=" << pck_received->get_source() << " cycle=" << local_cycle << "\n";
        ...
        {
            data_t data = this->temporal_register ? this->temporal_register->get_data() : 0;
            // std::cout << "[ACCUMULATOR] Accumulator " << this->n_accumulator << ". Current PSUM: " << data << "\n";
