...
TemporalRN::TemporalRN(id_t id, std::string name, Config stonne_cfg, Connection* outputConnection) : ReduceNetwork(id, name) {
    ...
    if(stonne_cfg.m_MSNetworkCfg.multiplier_network_type==WS_MESH) {
        this->accumulation_buffer_size = stonne_cfg.m_MSNetworkCfg.ms_cols;
    }
    else {
...
void TemporalRN::setMemoryConnections(std::vector<std::vector<Connection*>>  memoryConnections) {
    ...
    std::cout<<"n_bus_lines: "<<n_bus_lines<<" accumulation_buffer_size: "<<this->accumulation_buffer_size<<"\n";
...
void TemporalRN::configureSignals(Tile* current_tile, Region* region, unsigned int ms_size, unsigned int n_folding) {
    this->accumulationBuffer->configureSignals(current_tile, region, ms_size, n_folding);
...
void TemporalRN::NPSumsConfiguration(unsigned int n_psums) {
    this->accumulationBuffer->NPSumsConfiguration(n_psums);
}
