...
static void sp_tiled_matmul_os(const elem_t * A, const elem_t * B, const void * D, void * C,
  ...
  if (D != NULL && !no_bias) {
    ...
    for (size_t i = 0; i < I; i++) {
      for (size_t j = 0; j < J; j += D_blocks) {
        ...
        const elem_t * const D_dram_addr = (elem_t *)D + (bias_row * D_row_stride + j)*DIM;
        ...
        cosim_mvin_acc((address_t)D_dram_addr, D_sp_addr_acc, cols, rows);
      }
    }
  }
  ...
  for (size_t j = 0; j < J; j += B_blocks) {
    for (size_t k = 0; k < K; k++) {
      ...
      cosim_mvin((address_t)B_dram_addr, B_sp_addr, cols, rows);

      // printf("k, J, j, k*J +j = %zu, %zu, %zu, %zu\n", k, J, j, k*J + j);
    }
  }
  ...
  for (size_t i = 0; i < I; i++) {
    for (size_t k = 0; k < K; k += A_blocks) {
      ...
      const size_t rows = DIM - (i == I-1 ? pad_I : 0);
      cosim_mvin((address_t)A_dram_addr, A_sp_addr, cols, rows);
    }
  }
  ...
  for (size_t i = 0; i < I; i++) {
    for (size_t j = 0; j < J; j++) {
      ...
      for (size_t k = 0; k < K; k++) {
        ...
        // const uint32_t C_addr = (uint32_t)C + (i*C_row_stride + j)*DIM;
        ...
        // const uint32_t A_addr = (uint32_t)A + (i*A_row_stride + k)*DIM;
        // const uint32_t B_addr =  (uint32_t)B + (j*B_row_stride + k)*DIM;
        // uint32_t out_addr = C_addr;

        // printf("i=%u, j=%u, k=%u\n", i, j, k);
        // printf("A_row_stride = %u\n", A_row_stride);
        // printf("B_row_stride = %u\n", B_row_stride);
        // printf("C_row_stride = %u\n", C_row_stride);
        // printf("A offset = %u\n", (i*A_row_stride + k)*DIM);
        // printf("B offset = %u\n", (j*B_row_stride + k)*DIM);
        // printf("C offset = %u\n", (i*C_row_stride + j)*DIM);
        ...
        const size_t B_rows = DIM - (j == J - 1 ? pad_J : 0);
        const size_t B_cols = DIM - (k == K - 1 ? pad_K : 0);
        const size_t C_cols = DIM - (j == J - 1 ? pad_J : 0);
        const size_t C_rows = DIM - (i == I - 1 ? pad_I : 0);

        // printf("A_cols: %zu, A_rows: %zu, B_cols: %zu, B_rows: %zu, C_cols: %zu, C_rows: %zu\n",
        //        A_cols, A_rows, B_cols, B_rows, C_cols, C_rows);

        // gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr, DIM, DIM, C_cols, C_rows);
        // printf("GARBAGE_ADDR: %u, out_sp_addr: %u, C_cols: %zu, C_rows: %zu\n", GARBAGE_ADDR, out_sp_addr, C_cols, C_rows);

        // if (k == 0) { // First iteration
          // gemmini_extended_compute_preloaded(A_sp_addr, B_sp_addr, A_cols, A_rows, B_cols, B_rows);
          // printf("First iteration: A_sp_addr: %u, B_sp_addr: %u, A_cols: %zu, A_rows: %zu, B_cols: %zu, B_rows: %zu\n",
          //        A_sp_addr, B_sp_addr, A_cols, A_rows, B_cols, B_rows);
        // } else { // All other iterations
          // gemmini_extended_compute_accumulated(A_sp_addr, B_sp_addr, A_cols, A_rows, B_cols, B_rows);
          // printf("Other iteration: A_sp_addr: %u, B_sp_addr: %u, A_cols: %zu, A_rows: %zu, B_cols: %zu, B_rows: %zu\n",
          //        A_sp_addr, B_sp_addr, A_cols, A_rows, B_cols, B_rows);
        // }
        ...
        cosim_fake();
        if (k == K - 1) {
            // cosim_matrix_multiply(B_rows, A_cols, A_rows, A_addr, out_addr, B_addr, A_sp_addr, C_sp_addr, B_sp_addr);
            cosim_matrix_multiply(B_rows, A_cols, A_rows, 0, 0, 0, A_sp_addr, C_sp_addr, B_sp_addr);
        } else {
            cosim_matrix_multiply_stay(B_rows, A_cols, A_rows, 0, 0, 0, A_sp_addr, C_sp_addr, B_sp_addr);
            // cosim_matrix_multiply_stay(B_rows, A_cols, A_rows, A_addr, out_addr, B_addr, A_sp_addr, C_sp_addr, B_sp_addr);
        }
      }
    }
  }
  ...
  if (C != NULL) {
    ...
    for (size_t i = 0; i < I; i++) {
      for (size_t j = 0; j < J; j++) {
        void * const C_dram_addr = (elem_t*)C + (i*C_row_stride + j)*DIM*sizeof_C;
        const uint32_t C_sp_addr = C_sp_addr_start + (i*J + j)*DIM;

        const size_t C_cols = DIM - (j == J - 1 ? pad_J : 0);
        const size_t C_rows = DIM - (i == I - 1 ? pad_I : 0);

        cosim_mvout((address_t)C_dram_addr, C_sp_addr, C_cols, C_rows);
      }
    }
  }
}
...
static void sp_tiled_matmul_ws(const elem_t * A, const elem_t * B,
  ...
  if (D != NULL && !no_bias) {
    for (size_t i = 0; i < I; i++) {
      ...
      for (size_t j = 0; j < J; j += D_blocks) {
        const size_t bias_row = repeating_bias ? 0 : i;
        const void * const D_dram_addr = (elem_t *)D + (bias_row * D_row_stride + j)*DIM*sizeof_D;
        const uint32_t D_sp_addr_acc = D_sp_addr_start + (i*J + j)*DIM;
        // printf("offset D_sp_addr_acc=%u\n", (i*J + j)*DIM);
        size_t blocks = j + D_blocks <= J ? D_blocks : J-j;
        const size_t cols = blocks * DIM - (j + blocks >= J ? pad_J : 0);
        // gemmini_extended_mvin3(D_dram_addr, D_sp_addr_acc, cols, rows);

        // printf("D mvin addr=%x sp_addr=%u cols=%zu rows=%zu\n",
        //        (uint32_t)D_dram_addr, D_sp_addr_acc, cols, rows);
        cosim_mvin_acc((address_t)D_dram_addr, D_sp_addr_acc, cols, rows);
      }
    }
  }
  ...
  for (size_t k = 0; k < K; k++) {
    for (size_t j = 0; j < J; j++) {
      ...
      // size_t B_rows = DIM - (j == J - 1 ? pad_J : 0);
      // size_t B_cols = DIM - (k == K - 1 ? pad_K : 0);
      ...
      //     // printf("B_sp_addr offset: %u, cols: %zu, rows: %zu\n", (j * K + k) * DIM, cols, rows);
      //     cosim_mvin((address_t)B_dram_addr, B_sp_addr, cols, rows);
      // }

      // printf("addr B=%u %u %x %x\n",(uint32_t)B ,(uint32_t)B + (j*B_row_stride + k)*DIM, (uint32_t)B, (uint32_t)B + (j*B_row_stride + k)*DIM);

      // cosim_preload(B_rows, B_cols, 0, 0, 0, B_addr, GARBAGE_ADDR, GARBAGE_ADDR, B_sp_addr);
      ...
      for (size_t i = 0; i < I; i++) {
        ...
        // printf("i, j, k=%u %u %u\n", i, j, k);
        // printf("offset A_sp_addr=%u B_sp_addr=%u C_sp_addr=%u\n", (i*K + k)*DIM, (j*K + k)*DIM, (i*J + j)*DIM);
      ...
      // size_t B_cols = DIM - (k == K - 1 ? pad_K : 0);
        ...
        //   }
        // } else {
          if (A != NULL && j == 0 && k % A_blocks == 0) {
            const elem_t * const A_dram_addr = A + (i*A_row_stride + k)*DIM;
            const size_t blocks = k + A_blocks <= K ? A_blocks : K-k;
            const size_t cols = blocks * DIM - (k + blocks >= K ? pad_K : 0);
            const size_t rows = DIM - (i == I-1 ? pad_I : 0);
            // printf("A_sp_addr offset: %u, cols: %zu, rows: %zu\n", (i*K + k)*DIM, cols, rows);
            cosim_mvin((address_t)A_dram_addr, A_sp_addr, cols, rows);
          }
        // }
        // // Mvin B
        // if (b_transpose) {
          if (B != NULL && i == 0 && k % B_blocks == 0) {
            const elem_t * const B_dram_addr = B + (j*B_row_stride + k)*DIM;
            const size_t blocks = k + B_blocks <= K ? B_blocks : K-k;
            const size_t cols = blocks * DIM - (k + blocks >= K ? pad_K : 0);
            const size_t rows = DIM - (j == J-1 ? pad_J : 0);
      ...
      size_t B_cols = DIM - (k == K - 1 ? pad_K : 0);
          ...
          if (i == 0) {
            // cosim_preload(B_rows, B_cols, 0, 0, 0, B_addr, GARBAGE_ADDR, GARBAGE_ADDR, B_sp_addr);
            //   cosim_matrix_multiply_flip(B_rows, A_cols, A_rows, A_addr, out_addr, B_addr, A_sp_addr, out_sp_addr, B_sp_addr);
            cosim_preload(B_rows, B_cols, 0, 0, 0, 0, GARBAGE_ADDR, GARBAGE_ADDR, B_sp_addr);
              cosim_matrix_multiply_flip(B_rows, A_cols, A_rows, 0, 0, 0, A_sp_addr, out_sp_addr, B_sp_addr);
          } else {
              cosim_matrix_multiply(B_rows, A_cols, A_rows, 0, 0, 0, A_sp_addr, out_sp_addr, B_sp_addr);
              // cosim_matrix_multiply(B_rows, A_cols, A_rows, A_addr, out_addr, B_addr, A_sp_addr, out_sp_addr, B_sp_addr);
          }
        }
        ...
        if (C != NULL && k == K-1) {
          ...
          if (((act != LAYERNORM) && (act != SOFTMAX)) && (j == J-1 || j % C_blocks == C_blocks-1)) {
            ...
            void * const C_dram_addr = (elem_t*)C + (i*C_row_stride + rounded_j)*DIM*sizeof_C;
            const size_t blocks = rounded_j + C_blocks <= J ? C_blocks : J-rounded_j;
            const size_t cols = blocks * DIM - (rounded_j + blocks >= J ? pad_J : 0);
            const size_t rows = DIM - (i == I - 1 ? pad_I : 0);
            // printf("offset rounded_C_sp_addr: %u, cols: %zu, rows: %zu\n", (i*J + rounded_j)*DIM, cols, rows);
            // gemmini_extended_mvout(C_dram_addr, rounded_C_sp_addr, cols, rows);

            cosim_mvout((address_t)C_dram_addr, rounded_C_sp_addr, cols, rows);
          }
...
static void tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  printf("dim_I: %lu, dim_I_padded: %lu, last_I: %lu, tile_I: %lu\n", dim_I, dim_I_padded, last_I, tile_I);
  printf("dim_J: %lu, dim_J_padded: %lu, last_J: %lu, tile_J: %lu\n", dim_J, dim_J_padded, last_J, tile_J);
  printf("dim_K: %lu, dim_K_padded: %lu, last_K: %lu, tile_K: %lu\n", dim_K, dim_K_padded, last_K, tile_K);
  ...
  // cosim_config(DIM, DIM, DIM, (uint32_t)A, (uint32_t)C, (uint32_t)B, dim_J, dim_K, dim_I);
  cosim_config(DIM, DIM, DIM, 0, 0, 0, stride_C, stride_A, dim_I);
  cosim_fake();
  cosim_fake();
  cosim_fake();
  cosim_fake();
    ...
    for (size_t j0 = 0; j0 < J0; j0++)
      for (size_t k0 = 0; k0 < K0; k0++) {
        ...
        if (k0 != 0) {
          ...
          pre = (elem_t*)D + (bias_row * stride_D + j0 * tile_J * DIM)*sizeof_D;
        }

        void * out = k0 == K0-1 ? (elem_t*)C + (i0*tile_I*DIM*stride_C + j0*tile_J*DIM)*sizeof_C : NULL;

        const size_t I = i0 < I0-1 ? tile_I : last_I;
        const size_t J = j0 < J0-1 ? tile_J : last_J;
        const size_t K = k0 < K0-1 ? tile_K : last_K;

        // printf("Tiled Matmul Outer i0: %lu, j0: %lu, k0: %lu, I: %zu, J: %zu, K: %zu, xxx= %zu\n",
        //     i0, j0, k0,  I, J, K, BANK_NUM * BANK_ROWS - K * J * DIM);
        ...
        const elem_t * b = (B + j0*tile_J*DIM*stride_B + k0*tile_K*DIM);
...
#define UNDILATED(x) ((input_dilated) ? (((x)+1)/2) : (x))
      ...
      for (int orow = 0; orow < orows; orow++)
        for (int ocol = 0; ocol < ocols; ocol += DIM) {
          ...
          for (int och = 0; och < ochs; och += max_ochs_per_mvin) {
            ...
            cosim_mvin_acc((address_t)bias_dram_addr, D_sp_addr, J, I);
          }
        }
  }
  ...
  if (input != NULL){
    ...
    const int b_it = 1;
    const int ich_it = max_chs_per_mvin;
...
#define DS(x) ((x) >> (downsample))
            ...
            cosim_mvin((address_t)in, A_sp_addr, K, I >> downsample);
          }
  ...
  if (weights != NULL) {
    ...
    const size_t och_it = max_chs_per_mvin;
    const size_t kch_it = DIM;
      ...
      for (int krow = 0; krow < krows; krow++)
        for (int kcol = 0; kcol < kcols; kcol++)
          for (int kch = 0; kch < kchs; kch += kch_it) {
            ...
            cosim_mvin((address_t)w, B_sp_addr, J, K);
          }
    }
  }
            ...
            for (int b = 0; b < batches; b += b_it) {
              for (int orow = 0; orow < orows; orow++) {
                ...
                for (int ocol = 0; ocol < ocols;) {
                  ...
                  // Compute the corresponding DRAM addresses for this tile.
                  // These are needed by the cosim model (similar to A_addr/B_addr/out_addr in OS matmul).
                  // address_t A_addr = 0;
                  // address_t B_addr = 0;
                  // address_t out_addr = 0;

                  // if (input != NULL) {
                  //   // "input" passed into sp_tiled_conv already points to the top-left of the current slice.
                  //   // The compute phase indexes within that slice using (b, irow, icol, kch).
                  //   if (trans_input_3120) {
                  //     A_addr = (address_t)(input + (kch * in_row_dim * in_col_dim + DS(irow) * in_col_dim + DS(icol)) * batch_size + b);
                  //   } else {
                      // A_addr = (address_t)(input + (b * in_row_dim * in_col_dim + DS(irow) * in_col_dim + DS(icol)) * in_stride + kch);
                  //   }
                  // }

                  // if (weights != NULL) {
                    // cosim assumes B is laid out as N x K for matmul (row-major):
                    //   row = output-channel (och)
                    //   col = flattened kernel index (k = (krow,kcol,kch))
                    // Therefore pass a DRAM base pointer consistent with that view.
                    // NOTE: This only affects the cosim model (DRAM-side view); B_sp_addr is unchanged.

                    // const int K_total = kernel_dim * kernel_dim * in_channels;
                    // const int k_flat = (krow_ * kernel_dim + kcol_) * in_channels + kch;

                    // if (dw) {
                    //   // Depthwise weights don't match N x K; keep the simplest address (single filter per channel).
                    //   B_addr = (address_t)(weights + krow_ * kernel_dim + kcol_);
                    // } else {
                    //   // Base of B tile at row=och, col=k_flat in an implicit N x K matrix.
                    //   B_addr = (address_t)(weights + och * K_total + k_flat);
                    // }

                  // if (output != NULL) {
                  //   // "output" passed into sp_tiled_conv already points to the top-left of the current output slice.
                  //   // Layout matches the mvout loop below: (b, orow, ocol, och).
                  //   if (trans_output_1203) {
                  //     out_addr = (address_t)(output + (orow * out_col_dim * batch_size + ocol * batch_size + b) * out_channels + och);
                  //   } else {
                      // out_addr = (address_t)(output + (b * out_row_dim * out_col_dim + orow * out_col_dim + ocol) * out_stride + och);
                  //   }
                  // }
                  ...
                  // perform matmul (WS): preload weights (only when they change), then compute.
                  // In this co-sim harness, we model the effect via explicit calls.
                  if (new_weights) {
                    // cosim_preload(J, K, I, A_addr, out_addr, B_addr, GARBAGE_ADDR, C_sp_addr, B_sp_addr);
                    // cosim_matrix_multiply_flip(J, K, I, A_addr, out_addr, B_addr, A_sp_addr, C_sp_addr, B_sp_addr);
                    cosim_preload(J, K, I, 0, 0, 0, GARBAGE_ADDR, C_sp_addr, B_sp_addr);
                    cosim_matrix_multiply_flip(J, K, I, 0, 0, 0, A_sp_addr, C_sp_addr, B_sp_addr);
                  } else {
                    // cosim_matrix_multiply(J, K, I, A_addr, out_addr, B_addr, A_sp_addr, C_sp_addr, B_sp_addr);
                    cosim_matrix_multiply(J, K, I, 0, 0, 0, A_sp_addr, C_sp_addr, B_sp_addr);
                  }
        ...
        for (int orow = 0; orow < orows; orow++)
          for (int ocol = 0; ocol < ocols; ocol += DIM) {
            ...
            for (int och = 0; och < ochs; och += DIM) {
              ...
              cosim_mvout((address_t)out, C_sp_addr, J, I);
            }
          }
    } else {
      printf("Pooling with rectangular convolutions is currently not supported.\n");
...
static void tiled_conv(
  ...
  // Configure the co-simulation model for the upcoming WS conv.
  // Map convolution to an implicit GEMM:
  //   M = batch_size * out_row_dim * out_col_dim
  //   N = out_channels
  //   K = kernel_dim * kernel_dim * in_channels
  cosim_config(DIM, DIM, DIM,
    0, // (address_t)input,
    0, // (address_t)output,
    0, // (address_t)weights,
    (tile_t)out_channels,
    (tile_t)(kernel_dim * kernel_dim * in_channels),
    (tile_t)(batch_size * out_row_dim * out_col_dim));
...
static void tiled_conv_dw(
    ...
    // Configure the co-simulation model for the upcoming WS depthwise conv.
    // Map DW convolution to an implicit GEMM per-channel:
    //   M = batch_size * out_row_dim * out_col_dim
    //   N = channels
    //   K = kernel_dim * kernel_dim
    cosim_config(DIM, DIM, DIM,
      0, // (address_t)input,
      0, // (address_t)output,
      0, // (address_t)weights,
      (tile_t)channels,
      (tile_t)(kernel_dim * kernel_dim) * channels,
      (tile_t)(batch_size * out_row_dim * out_col_dim));
...
_STATIC void tiled_conv_auto(
    ...
    printf("running tiled_conv_auto\n");
