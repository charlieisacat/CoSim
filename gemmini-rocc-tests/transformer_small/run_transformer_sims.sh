#!/bin/bash
# Run all transformer_small runners and then run simulator on their renamed bitcode.
# Modeled after mobilenet/run_conv_sims.sh

# Do not exit on the first failure
# set -e

for src in *_matmul.c; do
#for src in matmul_ff1_matmul .c; do
  exe="${src%.c}"
  bc="${exe}_run_rn.bc"
